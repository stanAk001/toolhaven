...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center px-4 pt-[12vh]"
      ...
      <div className="relative w-full max-w-xl bg-paper border-2 border-ink rounded-2xl overflow-hidden" style={{ boxShadow: "8px 8px 0 var(--shadow-cast)" }}>
        ...
        <div className="relative flex items-center gap-3 px-5 py-4 border-b-2 border-ink">
          ...
          <kbd className="font-mono text-[10px] uppercase tracking-wide bg-paper2 border-2 border-ink rounded px-2 py-1 shrink-0">Esc</kbd>
        </div>
          ...
          {items.map((it, i) => (
            <li key={it.kind + it.to}>
              <button type="button" onMouseEnter={() => setActive(i)} onClick={() => go(it)}
                ...
                <span className="grid place-items-center w-9 h-9 rounded-lg border-2 border-ink font-display font-semibold text-white shrink-0"
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block font-mono text-[11px] uppercase tracking-wide text-ink2 truncate">{it.sub}</span>
                </span>
                <span className="font-mono text-[10px] uppercase tracking-wide text-ink2 shrink-0">{it.kind}</span>
              </button>
            </li>
          ))}
        </ul>
