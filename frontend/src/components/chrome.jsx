  ...
  return (
    <nav ref={navRef} onMouseLeave={settle} className="hidden md:flex items-center gap-1 relative">
      <span aria-hidden="true"
        className="absolute top-1 bottom-1 rounded-full bg-accent shadow-[0_6px_18px_rgba(232,67,31,.45)] transition-[left,width,opacity] duration-300 ease-[cubic-bezier(.2,.8,.2,1)]"
  ...
  return (
    <header className={`border-b-2 border-ink sticky top-0 z-40 transition-all duration-300 ${scrolled ? "nav-scrolled bg-paper/80 supports-[backdrop-filter]:bg-paper/70 backdrop-blur-md" : "bg-paper"}`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link to="/" className="group font-mono font-bold tracking-[.2em] text-lg inline-flex items-center">
          TOOLHAVEN
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button type="button" onClick={() => window.dispatchEvent(new Event("toolhaven:search"))}
            ...
            className="inline-flex items-center gap-2 rounded-full border-2 border-ink bg-paper px-3 py-1.5 transition-transform hover:-translate-y-0.5"
            style={{ boxShadow: "2px 2px 0 var(--shadow-cast)" }}>
            <Search size={15} strokeWidth={2.5} aria-hidden="true" />
            <span className="hidden lg:inline font-mono text-[11px] uppercase tracking-wide">Search</span>
            <kbd className="hidden lg:inline font-mono text-[10px] bg-paper2 border border-ink rounded px-1 leading-tight">⌘K</kbd>
          </button>
          <ThemeToggle />
          {/* the bars fold into an X when open */}
          <button className="md:hidden relative z-50 w-9 h-9 -mr-1" onClick={() => setOpen(!open)}
            aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open}>
          <span className={`absolute left-1.5 right-1.5 h-0.5 bg-ink transition-all duration-300 ease-[cubic-bezier(.2,.8,.2,1)] ${open ? "top-1/2 -translate-y-1/2 rotate-45" : "top-[11px]"}`} />
          <span className={`absolute left-1.5 right-1.5 top-1/2 -translate-y-1/2 h-0.5 bg-ink transition-all duration-200 ${open ? "opacity-0 scale-x-0" : "opacity-100"}`} />
          <span className={`absolute left-1.5 right-1.5 h-0.5 bg-ink transition-all duration-300 ease-[cubic-bezier(.2,.8,.2,1)] ${open ? "top-1/2 -translate-y-1/2 -rotate-45" : "bottom-[11px]"}`} />
          </button>
        </div>
      </div>
      ...
      {open && (
        <div className="md:hidden fixed inset-x-0 top-16 bottom-0 z-30 bg-paper flex flex-col fade-in">
          <nav className="flex-1 flex flex-col justify-center gap-1 px-8">
            {NAV.map(([to, label], i) => (
              <NavLink key={to} to={to} style={{ animationDelay: `${90 + i * 55}ms` }}
                className={({ isActive }) =>
                  `menu-rise group inline-flex items-center gap-3 font-display text-5xl sm:text-6xl font-semibold leading-[1.05] tracking-tight transition-colors ${isActive ? "text-accentDeep italic" : "hover:text-accentDeep"}`}>
          ...
          <div className="border-t-2 border-ink px-8 py-6 flex flex-wrap gap-2.5">
            {cats.map((c) => (
              <Link key={c.slug} to={`/category/${c.slug}`}
                style={{ "--cat": c.colorPrimary, boxShadow: "2px 2px 0 var(--shadow-cast)" }}
                className="group/m inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-wide px-3 py-1.5 border-2 border-ink rounded-full bg-paper transition-all duration-200 hover:-translate-y-0.5 hover:text-white hover:bg-[var(--cat)] hover:border-[var(--cat)] active:text-white active:bg-[var(--cat)]">
    ...
    <form onSubmit={submit} className="w-full max-w-md">
      <div className="flex items-center gap-2 border-2 border-ink rounded-full bg-paper pl-5 pr-1.5 py-1.5 transition-transform duration-200 focus-within:-translate-y-0.5"
        ...
        <button type="submit" disabled={status === "sending"} aria-label="Subscribe"
          className="group/sub grid place-items-center w-10 h-10 rounded-full bg-accent text-white shrink-0 shadow-[0_4px_12px_rgba(232,67,31,.4)] transition-transform duration-200 hover:scale-105 active:scale-95 disabled:opacity-60">
      ...
      <p className="font-mono text-[10px] uppercase tracking-[.18em] text-ink2 inline-flex items-center gap-1.5">
...
function FooterCol({ title, links }) {
  return (
    <div>
      <h4 className="font-mono text-[11px] uppercase tracking-[.2em] text-accentDeep mb-4">{title}</h4>
      <ul className="space-y-2.5">
        {links.map(([to, label]) => (
          <li key={to}>
            <Link to={to} className="group/l inline-flex items-center gap-1.5 font-display text-lg leading-tight hover:text-accentDeep transition-colors">
              {label}
...
export function Footer() {
  ...
  return (
    <footer className="relative border-t-2 border-ink mt-24 overflow-hidden bg-paper">
      <InkField className="absolute inset-0 pointer-events-none" gap={20} />
      <div className="relative max-w-6xl mx-auto px-6 py-14">
        <div className="grid lg:grid-cols-[1.3fr_1fr] gap-12 lg:gap-20">
          ...
          <div>
            <Link to="/" className="group inline-block">
              <span className="font-display font-semibold leading-[.85] tracking-tight block"
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {cats.map((c) => (
                  <Link key={c.slug} to={`/category/${c.slug}`}
                    style={{ "--cat": c.colorPrimary, boxShadow: "2px 2px 0 var(--shadow-cast)" }}
                    className="group/c inline-flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-wide px-3 py-1.5 border-2 border-ink rounded-full bg-paper transition-all duration-200 hover:-translate-y-0.5 hover:text-white hover:bg-[var(--cat)] hover:border-[var(--cat)]">
        ...
        <div className="relative mt-14 bg-ink text-paper rounded-xl border-2 border-ink px-6 py-6 md:px-8"
          ...
          <span aria-hidden="true"
            className="absolute -top-3 right-5 font-mono text-[10px] uppercase tracking-[.2em] bg-accent text-white px-2.5 py-1 border-2 border-ink rotate-3 select-none">
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
            ...
            <nav className="flex flex-wrap items-center font-mono text-[11px] uppercase tracking-[.18em]">
              {[["/privacy", "Privacy"], ["/disclosure", "Affiliate Disclosure"], ["/terms", "Terms"], ["/contact", "Contact"]].map(([to, label], i) => (
                <span key={to} className="inline-flex items-center">
                  ...
                  <Link to={to}
                    className="relative text-paper/80 hover:text-white transition-colors py-0.5
                      after:absolute after:left-0 after:right-0 after:-bottom-px after:h-0.5 after:bg-accent
            ...
            <Link to="/disclosure"
              className="group/d self-start lg:self-auto inline-flex items-center gap-2.5 font-mono text-[10px] uppercase tracking-[.16em]
          ...
          <div className="relative mt-6 pt-5 border-t border-paper/15 flex flex-col sm:flex-row sm:items-center justify-between gap-3
            font-mono text-[10px] uppercase tracking-[.2em] text-paper/55">
