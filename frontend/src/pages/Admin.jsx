...
const STATUS_COLOR = { pending: "#6A5F52", reviewing: "#1D4ED8", listed: "#15803D", declined: "#1747C7" };
...
export default function Admin() {
      ...
      <Seo title="Editor's desk" description="Private." path="/admin" noIndex />
        ...
        <form onSubmit={(e) => { e.preventDefault(); load(input.trim()); }} className="flex gap-2">
          <input type="password" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Admin token" autoFocus
            className="flex-1 border border-rule rounded-card bg-paper px-4 py-3 outline-none focus:border-accent" />
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 sm:px-6 py-10 sm:py-12 fade-in">
      ...
      {/* ---- SUBMISSIONS ---- */}
      {/* The submissions desk. Was a status-filter strip over a flat list,
          which showed what had arrived but gave nowhere to make a decision. */}
      {view === "submissions" && <SubmissionsAdmin token={token} />}
      ...
      {view === "reviews" && (
        <>
          ...
          <div className="space-y-4">
            {reviews.map((r) => (
              <div key={r.id} className="border border-rule rounded-card bg-paper p-5" >
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 font-mono text-label uppercase tracking-wide text-ink2">
                  ...
                  <span className="ml-auto flex gap-2">
                    <button onClick={() => approveReview(r.id)}
                      className="px-3 py-1.5 rounded-ui border border-rule bg-paper hover:bg-paper2 transition-colors" style={{ color: "#15803D" }}>Approve</button>
                    <button onClick={() => rejectReview(r.id)}
                      className="px-3 py-1.5 rounded-ui border border-rule bg-paper hover:bg-paper2 transition-colors text-accentDeep">Reject</button>
                  </span>
                </div>
              </div>
            ))}
      ...
      {view === "stacks" && (
        <>
          ...
          <div className="space-y-4">
            {stacks.map((s) => (
              <div key={s.id} className="border border-rule rounded-card bg-paper p-5" >
                <div className="flex flex-wrap gap-2 mb-3">
                  {s.tools.map((t) => (
                    <Link key={t.slug} to={`/tools/${t.slug}`} title={t.name}
                      className="grid place-items-center min-w-[2rem] h-8 px-2 rounded-ui border border-rule text-white font-display font-bold text-sm"
                      style={{ background: t.category?.colorPrimary || "#0E1116" }}>{t.logoMono || t.name[0]}</Link>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-2 font-mono text-label uppercase tracking-wide text-ink2">
                  ...
                  <span className="ml-auto flex gap-2">
                    <button onClick={() => approveStack(s.id)} className="px-3 py-1.5 rounded-ui border border-rule bg-paper hover:bg-paper2 transition-colors" style={{ color: "#15803D" }}>Approve</button>
                    <button onClick={() => rejectStack(s.id)} className="px-3 py-1.5 rounded-ui border border-rule bg-paper hover:bg-paper2 transition-colors text-accentDeep">Reject</button>
                  </span>
                </div>
              </div>
            ))}
