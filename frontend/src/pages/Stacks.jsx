...
function ToolTile({ t }) {
  return (
    <Link to={`/tools/${t.slug}`} title={t.name}
      className="grid place-items-center min-w-[2rem] sm:min-w-[2.2rem] h-8 sm:h-9 px-1.5 sm:px-2 rounded-ui border border-rule text-white font-display font-bold text-xs sm:text-sm transition-transform hover:-translate-y-0.5"
...
function StackCard({ stack, tilt = 0 }) {
  return (
    <figure className="relative bg-paper border border-rule rounded-card p-4 sm:p-5 rotate-[var(--r)] hover:rotate-0 transition-transform duration-300"
      style={{ "--r": `${tilt}deg` }}>
      ...
      <div className="mb-14">
        ...
        {open && status !== "sent" && (
          <Reveal as="form" onSubmit={submit} noValidate
            className="border border-rule rounded-card bg-paper p-5 md:p-6 space-y-5" >
                  ...
                  {all.map((t) => {
                    ...
                    return (
                      <button type="button" key={t.slug} onClick={() => toggle(t.slug)} disabled={disabled}
                        className={`font-mono text-xs uppercase px-3.5 py-1.5 rounded-ui border border-rule transition-colors ${on ? "text-white" : "bg-paper hover:bg-paper2"} ${disabled ? "opacity-40" : ""}`}
        ...
        {status === "sent" && (
          <div className="border border-rule rounded-card bg-paper2/50 p-5">
